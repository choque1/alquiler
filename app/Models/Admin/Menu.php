    ...
    public function getHijos ($padres, $line){
        ...
        foreach($padres as $line1){
            if ($line['id']== $line1['menu_id']) {
                $children = array_merge($children, [array_merge($line, ['submenu' => $this-> getHijos($padres, $line1)])]);
            }
        }
        return $children;
    }
    ...
    public function getPadres($front){
        if($front){
            return $this->whereHas('roles', function($query){
                $query->where('rol_id', session()->get('rol_id'))->ordeby('menu_id');
            })->where('estado', 1)
               ->orderby('menu_id')
               ->orderby('orden')
               ->get()
